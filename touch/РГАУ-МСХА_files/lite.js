      ...
      function () {
        ...
        _createClass(AjaxProtoLegacy, [{
          ...
          value: function parseResponse(response) {
            ...
            if (!trim(text).length) {
              var data = [8, getLang('global_unknown_error')];
...
function setCookie(name, value, days) {
  ...
  var sameSite = browser.chrome && partConfigEnabled('cookie_class_samesite') ? '; SameSite=None' : '';
  document.cookie = name + '='+escape(value) + expires + '; path=/' + (domain ? '; domain=.' + domain : '') + sameSite;
}
